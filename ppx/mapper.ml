...
let default_auto, default_remove, default_threads =
  ...
  | env ->
    ...
    (List.mem "auto" opts, List.mem "remove" opts, List.mem "threads" opts)
...
let get_payload key = function
  ...
  | { attr_name = { txt; _ }
    ; attr_payload = PStr [ { pstr_desc = Pstr_eval (expression, _); _ } ]
    ; _
    }
    when txt = key ->
  ...
  | { attr_name = { txt; _ }; attr_payload = PStr []; _ } when txt = key ->
    Some None
  | { attr_name = { txt; _ }; attr_loc; _ } when txt = key ->
    error attr_loc `Payload_not_an_expression
  | _ -> None
      ...
      (Exp.let_ Nonrecursive
         [ Vb.mk
             ...
             (Exp.try_ expr
                [ Exp.case
                    (Pat.var (mknoloc "e"))
                    (Exp.sequence (exit_landmark landmark)
                       (Exp.apply (var raise_ident) [ (Nolabel, var "e") ]) )
                ] )
         ]
         (Exp.sequence (exit_landmark landmark) (var "r")) )
  in
  match landmark with
...
let rec wrap_landmark_method ctx landmark loc ({ pexp_desc; _ } as expr) =
  ...
  | Pexp_fun (label, def, pat, e) ->
    { expr with
      pexp_desc =
        Pexp_fun (label, def, pat, wrap_landmark_method ctx landmark loc e)
    }
  | Pexp_poly (e, typ) ->
    { expr with
      pexp_desc = Pexp_poly (wrap_landmark_method ctx landmark loc e, typ)
    }
...
let translate_value_bindings ctx value_binding auto vbs =
  let vbs_arity_name =
    List.map
      (fun vb ->
        match (vb, has_landmark_attribute ~auto vb.pvb_attributes) with
        | { pvb_expr; pvb_loc; pvb_pat; _ }, Some attr
          ...
          let from_names arity fun_name landmark_name =
            ...
            else (vb, Some (arity, fun_name, landmark_name, pvb_loc, attr))
          in
          match
            ( name_of_pattern pvb_pat
            , filter_map (get_payload "landmark") vb.pvb_attributes )
          with
          | Some fun_name, [] | Some fun_name, [ None ] ->
    ...
    filter_map
      (function
        | _, Some ((_ :: _ as arity), fun_name, landmark_name, loc, _) ->
          ...
          let expr =
            eta_expand (wrap_landmark ctx landmark_name loc) ident arity
          in
...
let mapper =
  object (self)
    ...
    method! module_binding ({ pmb_name; _ } as binding) ((auto, ctx) as acc) =
      let acc =
        match pmb_name.txt with None -> acc | Some txt -> (auto, txt :: ctx)
      in
        ...
        List.fold_left
          (fun (auto, acc) expr ->
            ...
            | { pstr_desc = Pstr_value (rec_flag, vbs); pstr_loc } ->
              let value_binding vb = fst (self#value_binding vb (auto, ctx)) in
              let vbs, new_vbs =
    ...
    method! class_field class_field ((auto, ctx) as acc) =
      ...
      | { pcf_desc = Pcf_method (loc, privat, Cfk_concrete (flag, expr))
        ...
        | Some landmark ->
          let expr =
            wrap_landmark_method ctx landmark pcf_loc
              (fst (self#expression expr acc))
          in
          ( { class_field with
              pcf_desc = Pcf_method (loc, privat, Cfk_concrete (flag, expr))
    ...
    method! expression expr ((_, ctx) as acc) =
      ...
      | [ Some landmark_name ] ->
        ( { expr with
            pexp_attributes = remove_attribute "landmark" pexp_attributes
          }
        ...
        List.filter
          (function
            | { pstr_desc = Pstr_attribute attr; _ }
              when has_landmark_attribute [ attr ] <> None ->
              false
            | _ -> true )
          l
      in
        ...
        if disable then l
          ...
          let module_name =
            Filename.remove_extension
              (Filename.basename !Ocaml_common.Location.input_name)
          in
          ...
          let lm =
            if auto then Some (new_landmark landmark_name first_loc) else None
          in
          if !landmarks_to_register = [] then l
          else
            let landmarks =
              Str.value Nonrecursive
                (List.map
                   (fun (landmark, landmark_name, landmark_location, id) ->
                     Vb.mk
                       (Pat.var (mknoloc landmark))
                       (register_constant_landmark ~id landmark_name
                          landmark_location ) )
            ...
            | Some lm ->
              let begin_load =
                Str.value Nonrecursive
                  [ Vb.mk
                      (Pat.construct (mknoloc (Longident.parse "()")) None)
                      (enter_landmark lm)
                  ]
              in
              let exit_load =
