...
let () =
  ...
  Printf.printf
    "Measuring the time spent between two consecutives call to clock (using %d \
     samples)...\n\
     %!"
    nb_samples;
  ...
  let avg_sample =
    Array.fold_left ( +. ) 0.0 gap_samples /. float (Array.length gap_samples)
  in
  let sq x = x *. x in
  let std_dev =
    Array.fold_left (fun acc x -> acc +. sq (x -. avg_sample)) 0.0 gap_samples
