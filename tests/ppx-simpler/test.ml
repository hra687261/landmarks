(** This test is a bit like test/ppx except we do not use the 'let[@landmark]
    ...' construction * which was not available in ocaml 4.02. *)

let rec fib n =
  (if n <= 1 then 1 else fib (n - 1) + fib (n - 2)) [@landmark "fib"]

let rec even n = match[@landmark "even"] n with 0 -> even0 | n -> odd (n - 1)

and odd n = match[@landmark "odd"] n with 0 -> odd0 | n -> even (n - 1)
...
let local_module () =
  let module M = struct
    ...
    let local2 () = print "local2" [@landmark "local2"]
