let[@landmark] rec fib n = if n <= 1 then 1 else fib (n - 1) + fib (n - 2)

let[@landmark] rec even = function 0 -> even0 | n -> odd (n - 1)

and[@landmark] odd = function 0 -> odd0 | n -> even (n - 1)
...
let[@landmark] rec next_prime n = if prime n then n else next_prime (n + 1)

[@@@landmark "auto"]

let print s = Printf.printf "in: %s\n%!" s
...
let local_module () =
  let module M = struct
    ...
    let[@landmark] local2 () = print "local2"

    let local_noauto () = print "local_noauto"
  end in
  M.local1 ();
  M.local2 ();
  M.local_noauto ()
...
class c =
  ...
  object (self)
    method m1 = print "m1" [@@landmark]

    method m2 () = print "m2" [@@landmark]

    method m3 () = print "m3" [@@landmark]
...
let () =
  ...
  let[@landmark] main () =
    ...
    object
      method immediate () = print "immediate" [@@landmark]
    end
      #immediate ();
