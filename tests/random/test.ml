...
module LM = struct
  ...
  let randoms =
    Array.init 100 (fun k -> L.register (Printf.sprintf "random_%d" k))
end
...
let rec fr n =
  ...
  let r = if n = 0 then 1.0 else fr (n - 1) *. exp (float n) in
  L.exit LM.recursive;
  r

let small k n x =
  if x > 1e-3 then Printf.eprintf "diff%d(%d):%g is not very small.\n%!" k n x
...
let reachable_landmarks graph =
  ...
  dfs
    (fun _ node ->
      result := node.landmark_id :: !result;
      true )
    (fun _ _ -> ())
    graph;
...
let checks () =
  ...
  if total <> aggregated_total then begin
    Printf.printf
      "Bug in aggregation primitive (%d calls before, %d after).\n%!" total
      aggregated_total;
    assert false
  end;
